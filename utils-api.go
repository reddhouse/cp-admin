...
var testEmailAddress = "notebeyondstate@email.com"
var testUserId = "01HP0B15Z5KFV3F050AWZZZKGN"
var testCode = 800227
...
func signup() {
	...
	var jsonData []byte
	if testEmailAddress == "" {
		jsonData = []byte(`{"email":"` + generateRandomEmailAddress() + `"}`)
	} else {
		jsonData = []byte(`{"email":"` + testEmailAddress + `"}`)
	}
...
func login() {
	...
	jsonData := []byte(fmt.Sprintf(`{"email":"%s"}`, testEmailAddress))
	// Send POST request using the default http client.
	resp, err := http.Post(url, "application/json", bytes.NewBuffer(jsonData))
	if err != nil {
		log.Fatalf("[error-admin] reading response: %v", err)
...
func loginCode() {
	...
	jsonData := []byte(fmt.Sprintf(`{"userId":"%s","code":%d}`, testUserId, testCode))
	// Send POST request using the default http client.
	resp, err := http.Post(url, "application/json", bytes.NewBuffer(jsonData))
	if err != nil {
		log.Fatalf("[error-admin] reading response: %v", err)
